  ...
  return (
    <div className="min-h-screen text-slate-100">
      <header className="flex items-center justify-between px-6 py-5 border-b border-slate-800">
        <div>
          <h1 className="text-xl font-semibold">walletsandmore</h1>
          <p className="text-sm text-slate-400">{APP_CHAIN_NAME} starter</p>
        </div>
        <ConnectButton />
      </header>
      ...
      <main className="mx-auto max-w-3xl px-6 py-10 space-y-6">
        <section className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
          ...
          <div className="mt-4 space-y-2 text-sm text-slate-300">
            ...
            <p>
              Block number:{' '}
              {isConnected ? (isLoading ? 'loading…' : blockNumber?.toString() ?? '—') : '—'}
            </p>
          </div>
        </section>
